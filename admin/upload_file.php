<?php
	...
	function resize($photo){
		...
		imagedestroy($src);
		imagepng($dst,$photo['tmp_name']); // adjust format as needed
		imagedestroy($dst);			
	}
		...
		if (isset($_FILES["photos"])) {
			...
			foreach ($photos as $photo) {
				...
				if (in_array($ext,$allowedExts) && in_array($photo["type"],$types) && $photo["size"] < 10000000 && $photo["error"] == 0) {
					...
					if (file_exists("../vehicles/" . $id . "/" . $photo["name"])) {									//Check if photo exists
						...
						$filename = $photo['name'];
						move_uploaded_file($photo["tmp_name"], "../vehicles/" . $id . "/" . $photo["name"]);		//Upload if false
						$pinsert->execute();																		//Insert into database
						echo $inc . ". <b>Photo Successfully Saved</b>: " . $photo["name"] . "<br><br>";			//Notify of success
					}
				} else {
					if ($photo["error"] == 4) {echo $inc . ".<br><br>";
					} else {
						$wsize = "";
						$wtype = "";
			...
			foreach ($files as $file) {
				...
				if ($file["size"] < 10000000 && $file["error"] == 0) {
					if (!file_exists("../vehicles/".$id)) {mkdir("../vehicles/".$id);}								//Check if folder exists - create if false
					if (file_exists("../vehicles/" . $id . "/" . $file["name"])) {									//Check if file exists
						echo $inc . ". <b>File Already Exists</b>: " . $file["name"] . "<br><br>";					//Notify if true
					} else {
						$filename = $file['name'];
						move_uploaded_file($file["tmp_name"], "../vehicles/" . $id . "/" . $file["name"]);		//Upload if false
						$finsert->execute();																		//Insert into database
						echo $inc . ". <b>File Successfully Saved</b>: " . $file["name"] . "<br><br>";			//Notify of success
					}
				} else {
	...
	echo "\n<p>Go Back to <a href='edit.php?id=".$id."'>".trim($rows[0]['year']." ".$rows[0]['make']." ".$rows[0]['model']." ".$rows[0]['trim'])."</a>.</p></div></body>";
?>